...
Run with `inference.py` using `Qwen/Qwen2.5-72B-Instruct` via HuggingFace router (zero-shot, up to 5 attempts with feedback):
...
HF_TOKEN=hf_... \
SERVER_URL=https://khyaal-d-sql-query-env.hf.space \
python inference.py
...
├── baseline.py      # Legacy entry point (aliases inference.py)
├── server/
│   └── app.py       # Multi-mode deployment entry point (re-exports app)
├── Dockerfile       # Container definition (port 7860, ENABLE_WEB_INTERFACE=true)
├── requirements.txt # Python dependencies
├── pyproject.toml   # Package config with [project.scripts] for multi-mode deploy
├── uv.lock          # Locked dependency versions (generated by uv)
├── openenv.yaml     # Environment manifest (name, author, version)
└── README.md
```
